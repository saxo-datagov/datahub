...
function check_reindex() {
    initial_documents=$(curl -XGET "$ELASTICSEARCH_HOST:$ELASTICSEARCH_PORT/$1/_count" -H 'Content-Type: application/json' | jq '.count')
    for i in $(seq 30); do
      reindexed_documents=$(curl -XGET "$ELASTICSEARCH_HOST:$ELASTICSEARCH_PORT/$2/_count" -H 'Content-Type: application/json' | jq '.count')
...
function reindex() {
  ...
  curl -XPUT "$ELASTICSEARCH_HOST:$ELASTICSEARCH_PORT/$target_index" -H 'Content-Type: application/json' --data @/tmp/data
  ...
  curl -XPOST "$ELASTICSEARCH_HOST:$ELASTICSEARCH_PORT/_reindex?pretty" -H 'Content-Type: application/json' \
  ...
  then
    ...
    if [ $(curl -o /dev/null -s -w "%{http_code}" "$ELASTICSEARCH_HOST:$ELASTICSEARCH_PORT/_alias/$source_index") -eq 404 ]
    then
      curl -XDELETE "$ELASTICSEARCH_HOST:$ELASTICSEARCH_PORT/$source_index"
    else
      concrete_index_name=$(curl -XGET "$ELASTICSEARCH_HOST:$ELASTICSEARCH_PORT/_alias/$source_index" | jq 'keys[]' | head -1 | tr -d \")
      curl -XDELETE "$ELASTICSEARCH_HOST:$ELASTICSEARCH_PORT/$concrete_index_name"
    fi

    curl -XPOST "$ELASTICSEARCH_HOST:$ELASTICSEARCH_PORT/_aliases" -H 'Content-Type: application/json' \
      -d "{\"actions\":[{\"remove\":{\"index\":\"*\",\"alias\":\"$source_index\"}},{\"add\":{\"index\":\"$target_index\",\"alias\":\"$source_index\"}}]}"

    echo -e "\nReindexing to $target_index succeded"
    return 0
  else
    curl -XDELETE "$ELASTICSEARCH_HOST:$ELASTICSEARCH_PORT/$target_index"
...
function create_index() {
  ...
  if [ $(curl -o /dev/null -s -w "%{http_code}" "$ELASTICSEARCH_HOST:$ELASTICSEARCH_PORT/$1") -eq 404 ]
  then
    echo -e '\ncreating index' "$1"
    ...
    curl -XGET "$ELASTICSEARCH_HOST:$ELASTICSEARCH_PORT/$1/_settings" | jq '.. | .settings? | select(. != null)' | \
      jq '. | {"index":{"max_ngram_diff": .index.max_ngram_diff, "analysis": .index.analysis}}' > /tmp/existing_setting
    curl -XGET "$ELASTICSEARCH_HOST:$ELASTICSEARCH_PORT/$1/_mapping" | jq '.. | .mappings? | select(. != null)' > /tmp/existing_mapping
